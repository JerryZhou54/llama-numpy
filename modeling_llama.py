...
class LlamaRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6, use_numpy=False):
        """
...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.use_numpy = config.use_numpy
        # if self.use_numpy:
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            if self.use_numpy:
                gate_out = np.dot(x, self.gate_proj.weight.detach().cpu().numpy().T)
                up_out = np.dot(x, self.up_proj.weight.detach().cpu().numpy().T)
                act = self.silu(gate_out) * up_out
                output = np.dot(act, self.down_proj.weight.detach().cpu().numpy().T)
            else:
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            if self.use_numpy:
                query_states = np.dot(hidden_states, self.q_proj.weight.detach().cpu().numpy().T)
                key_states = np.dot(hidden_states, self.k_proj.weight.detach().cpu().numpy().T)
                value_states = np.dot(hidden_states, self.v_proj.weight.detach().cpu().numpy().T)
            else:
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.input_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps, use_numpy=config.use_numpy)
        self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps, use_numpy=config.use_numpy)
    ...
    def forward(
        ...
        hidden_states = torch.from_numpy(hidden_states).to(self.self_attn.q_proj.weight.dtype)
        residual = torch.from_numpy(residual).to(hidden_states.dtype)
